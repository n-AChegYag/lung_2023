...
class ToTensor:
    def __init__(self, mode='train'):
        if mode not in ['train', 'valid', 'test', 'predict']:
            raise ValueError(f"Argument 'mode' must be 'train', 'valid', 'test' or 'predict'. Received {mode}")
        self.mode = mode
...
class ToTensorOneModal:
    def __init__(self, mode='train'):
        if mode not in ['train', 'valid', 'test']:
            raise ValueError(f"Argument 'mode' must be 'train', 'valid' or 'test'. Received {mode}")
        self.mode = mode
...
class NormalizeIntensity:
    ...
    @staticmethod
    def normalize(img):
        norm_img = np.clip(img, -512, 512) / 512
        return norm_img
