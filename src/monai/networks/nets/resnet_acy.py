...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv_1x1 = nn.Conv3d(block_inplanes[3], block_inplanes[3] // 4, 1, 1)
        self.avgpool = avgp_type(block_avgpool[spatial_dims])
        self.fc = nn.Linear(block_inplanes[3] // 4 * block.expansion + c_dim, num_classes) if feed_forward else None
    ...
    def forward(self, x, c: torch.Tensor) -> torch.Tensor:
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
