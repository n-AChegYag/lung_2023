...
from .SE_layers import BasicConv3d, FastSmoothSeNormConv3d, RESseNormConv3d, UpConv
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # if self.n_cls == 1:
        #     return torch.sigmoid(x), class_logit
        # else:
        #     return F.softmax(x, dim=1), class_logit
        return x, class_logit
...
class EncoderCAM(nn.Module):
    def __init__(self, in_channels, n_cls, n_filters):
        super(EncoderCAM, self).__init__()
        self.in_channels = in_channels
        self.n_cls = 1 if n_cls == 2 else n_cls
        self.n_filters = n_filters
...
if __name__ == '__main__':
    ...
    seunet = SEUNet(2,1,8)
    seunet.cuda()
    image = np.zeros((4,2,32,144,144)).astype('float32')
    image = torch.from_numpy(image)
    image = image.cuda()
    pred, class_logit = seunet(image)
    print(
        'Input size: {}\n'
        'Output size: {}\n'
        'class_logit: {}'.format(
